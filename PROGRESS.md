...
- [x] **Windows build produced** by cross-compiling on Linux (`cargo-xwin`, target `x86_64-pc-windows-msvc`, NSIS via `makensis`): `StudyHub_0.1.0_x64-setup.exe` (per-user installer) and the portable `studyhub.exe`. Not code-signed (no certificate) – SmartScreen shows a warning on first start. The MSI (WiX) can only be produced on a Windows host or by the workflow.
- [x] **Android APK produced**: `tauri android init` + `tauri android build --apk` with SDK 34 / NDK 27; universal release APK signed with a freshly generated keystore (`upload-keystore.jks`, alias `studyhub`), handed to the project owner together with the passwords. The generated Android project (`src-tauri/gen/android`) is committed; keystore and `keystore.properties` are git-ignored.
- [x] Signing helper fixed during the real build: no duplicate `Properties` import, keystore path resolved with `rootProject.file(...)`
